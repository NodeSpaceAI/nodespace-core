...
/// mechanics. The eligibility gate and the collision rule are stated up front
/// because both are checkable before calling, and because discovering either
/// only through a rejection reads as the capability not existing.
pub const ADD_ENUM_VALUES: SchemaRule = SchemaRule {
    id: "add-enum-values",
    imperative: "ADDING A VALUE TO AN EXISTING ENUM \u{2014} use add_field_values, NOT add_fields: When the user wants a new choice on a field that already exists (a \"backlog\" status on task, a new priority level), call update_schema with add_field_values: `{\"schema_id\": \"task\", \"add_field_values\": [{\"field\": \"status\", \"values\": [{\"value\": \"backlog\", \"label\": \"Backlog\"}]}]}`. This appends to the EXISTING field's vocabulary. Do NOT use add_fields \u{2014} that declares a NEW field, which is the wrong operation and leaves the original field unchanged. Do NOT try to redeclare the field with a fuller coreValues list either; that is rejected. ELIGIBILITY: only a field with extensible: true AND type enum can be extended \u{2014} check with get_schema_definition first rather than guessing. New values land in user_values; core_values is never touched. COLLISIONS: the whole call is rejected if the field does not exist, is not extensible, is not an enum, or if any value string already exists in core_values or user_values \u{2014} nothing is merged or overwritten. The check is on the value string, never the label: two values may share a label, so a rejection naming a colliding value means choose a different value string, not a different label.",
