...
/// 2. Simulates a session where a new schema is added between turns
/// 3. Verifies the inference engine loads and executes correctly
/// 4. Validates that the model receives the refreshed context for the second turn
///
...
async fn test_real_inference_loads_and_runs() {
    ...
    if !model_path.exists() {
        eprintln!(
            "⏭️  Skipping real inference test: model not found at {:?}",
            model_path
        );
        return;
    }
