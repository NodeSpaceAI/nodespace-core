...
**Close out the ending cycle** — On the day a cycle ends, create its successor — starting the next day and spanning that cycle's own duration_days — then move the ending cycle's tasks into it. Every task moves, finished ones included: the engine has no per-item filter for a for_each yet.
